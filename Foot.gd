...
func _ready():
	var file = File.new()
	file.open("res://assets/character/foot/foot.json", file.READ)
	# Parse the json and store it
	playerJson = parse_json(file.get_as_text())
	role_action="stand"
	var png_key = "{my_part}_{role_action}_3_0".format({"my_part":my_part,"role_action":role_action})
	var rect_info = playerJson["frames"][png_key]["frame"]
	region_rect= Rect2(rect_info["x"],rect_info["y"],rect_info["w"],rect_info["h"])

func change_foot(direction:String, now_y:String):
	if direction == "idle":
		role_action = "stand"
		now_y = direction_map[now_y]
		var png_key = "{my_part}_{role_action}_{now_x}_{now_y}".format({
			"my_part":my_part,
